/*
  ...
  1. New Tables
    - `llm_models` - Stores configuration for different LLM models including API keys, URLs, and invocation order
  2. Security
    - Enable RLS on the table
    - Add policies for admin access and read-only for regular users
*/
...
FOR INSERT
WITH CHECK (coalesce(role(), '') = 'supabase_admin');
...
FOR DELETE
USING (coalesce(role(), '') = 'supabase_admin');
